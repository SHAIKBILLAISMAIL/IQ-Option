...
const categories = [
  { id: "trending", name: "Trending", icon: "🔥", count: 0 },
  { id: "options", name: "Options", icon: "⚡", count: 51 },
  { id: "forex", name: "Forex", icon: "$", count: 42 },
  { id: "stocks", name: "Stocks", icon: "🏢", count: 254 },
  { id: "crypto", name: "Crypto", icon: "₿", count: 44 },
  { id: "commodities", name: "Commodities", icon: "⚪", count: 9 },
  { id: "indices", name: "Indices", icon: "📊", count: 12 },
  { id: "etfs", name: "ETFs", icon: "📦", count: 27 },
  { id: "watchlist", name: "Watchlist", icon: "⭐", count: 0 },
];

// Asset definitions removed in favor of centralized config
...
function TradeContent() {
  ...
  const positionsListRef = useRef<HTMLDivElement | null>(null);
  ...
  // Reload account-specific data when switching between practice/real
  useEffect(() => {
    if (session?.user) {
      loadOpenPositions(accountType);
      loadTradingHistory(accountType);
    }
  }, [session, accountType]);
  ...
  const loadOpenPositions = async (type: "practice" | "real" = accountType) => {
    try {
      const token = localStorage.getItem("bearer_token");
      const response = await fetch(`/api/trades?account_type=${type}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const loadTradingHistory = async (type: "practice" | "real" = accountType) => {
    try {
      const token = localStorage.getItem("bearer_token");
      const response = await fetch(`/api/trading-history?account_type=${type}&limit=20`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  // Set initial selected asset only once when no asset is selected
  useEffect(() => {
    if (currentAssets.length > 0 && !selectedAsset) {
      setSelectedAsset(currentAssets[0]);
    }
  }, [currentAssets.length]); // Only depend on length, not the whole array

  // Update selected asset prices when market data changes
  useEffect(() => {
    if (selectedAsset) {
      const updatedData = marketData[selectedAsset.name];
      if (updatedData) {
        // Only update price data, not the whole object to avoid loops
        setSelectedAsset(prev => {
          if (!prev) return prev;
          // Only update if prices actually changed
          if (prev.buy === updatedData.ask && prev.sell === updatedData.bid) {
            return prev;
          }
          return {
            ...prev,
            ...
            isMock: updatedData.isMock,
          };
        });
      }
    }
  }, [marketData]); // Only depend on marketData, not selectedAsset

  // Calculate total P&L
  const totalPnL = positions.reduce((sum, pos) => sum + pos.pnl, 0);
  const currentBalance = accountType === "practice" ? balance : realBalance;

  // Keep trade amount within available balance
  useEffect(() => {
    if (currentBalance <= 0) {
      setTradeAmount(0);
      return;
  ...
  const handleTrade = async (direction: "buy" | "sell") => {
    ...
    if (currentBalance <= 0) {
      toast.error("Your balance is $0. Please deposit funds before trading.");
    ...
    try {
      ...
      const response = await fetch("/api/trades", {
        ...
        body: JSON.stringify({
          ...
          assetType: resolveAssetType(selectedCategory),
          direction,
          amount: tradeAmount,
          entryPrice,
          currentPrice,
      ...
      requestAnimationFrame(() => {
        positionsListRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
      });
  ...
  const handleDepositSuccess = (amount: number) => {
    ...
    // 2. Reload data after a short delay to ensure DB propagation
    setTimeout(() => {
      loadUserBalance();
      loadOpenPositions(accountType);
      loadTradingHistory(accountType);
  ...
  const handleWithdrawSuccess = (amount: number) => {
    ...
    // 2. Reload data after a short delay
    setTimeout(() => {
      loadUserBalance();
      loadOpenPositions(accountType);
      loadTradingHistory(accountType);
    ...
    <div className="min-h-screen bg-[#1a1d29] text-white">
      ...
      <header className="bg-[#0d0f15] border-b border-[#2a2d3a] h-14 flex items-center px-4">
        <Link href="/" className="flex items-center gap-2 mr-6">
          <div className="w-6 h-6 bg-[#ff8516] rounded flex items-center justify-center">
        ...
        <div className="flex items-center gap-1 flex-1 overflow-x-auto">
          {selectedAsset && (
            <button className="flex items-center gap-2 px-3 py-1.5 bg-[#1a1d29] rounded text-sm hover:bg-[#2a2d3a] transition-colors">
          ...
          {openTabs.map((tab, idx) => (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded text-sm transition-colors ${selectedAsset?.name === tab.name
                ? "bg-[#ff8516] text-white"
            ...
            >
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setOpenTabs(openTabs.filter((_, i) => i !== idx));
                  if (selectedAsset?.name === tab.name) {
                    setSelectedAsset(openTabs[0] || null);
                  }
                }}
                className="hover:text-orange-500"
              >
          ...
          <button
            onClick={() => {
              if (selectedAsset && !openTabs.find(t => t.name === selectedAsset.name)) {
                setOpenTabs([...openTabs, selectedAsset]);
              }
            }}
            className="p-1.5 hover:bg-[#1a1d29] rounded"
            title="Open new tab"
          >
            <Plus size={16} />
          </button>
        ...
        <div className="flex items-center gap-2 mr-4 bg-[#1a1d29] rounded-lg p-1">
          <button
            ...
            className={`px-3 py-1.5 rounded text-xs font-semibold transition-colors ${accountType === "practice" ? "bg-[#ff8516] text-white" : "text-gray-400 hover:text-white"
              }`}
          >
            Practice
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded text-xs font-semibold transition-colors ${accountType === "real" ? "bg-[#00c853] text-white" : "text-gray-400 hover:text-white"
              }`}
          >
            Real
          </button>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-[#ff8516] rounded-full flex items-center justify-center text-white font-semibold text-sm">
              {session?.user?.email?.[0]?.toUpperCase() || 'U'}
            </div>
            <div className="text-right">
              <div className="flex items-center gap-3 text-[11px] text-gray-400 mb-0.5">
                <span className={accountType === "practice" ? "text-[#ff8516] font-semibold" : ""}>
                  Practice: ${balance.toFixed(2)}
                </span>
                <span className={accountType === "real" ? "text-[#00c853] font-semibold" : ""}>
          ...
          <button
            ...
            className="px-4 py-1.5 bg-[#1a1d29] hover:bg-[#2a2d3a] rounded text-sm font-semibold flex items-center gap-2 border border-[#2a2d3a]"
          >
            <Banknote size={16} />
            Withdraw
          </button>
      ...
      <div className="flex h-[calc(100vh-56px)]">
        ...
        <div className="w-16 bg-[#0d0f15] border-r border-[#2a2d3a] flex flex-col items-center py-4 gap-4">
          ...
          <div className="flex flex-col gap-1 my-4">
            <button
              ...
              className={`px-2 py-1 text-xs rounded ${chartInterval === "1" ? "bg-[#ff8516] text-white" : "hover:bg-[#1a1d29]"}`}
            >
              1m
            </button>
            <button
              onClick={() => setChartInterval("60")}
              className={`px-2 py-1 text-xs rounded ${chartInterval === "60" ? "bg-[#ff8516] text-white" : "hover:bg-[#1a1d29]"}`}
            >
              1h
            </button>
            <button
              onClick={() => setChartInterval("D")}
              className={`px-2 py-1 text-xs rounded ${chartInterval === "D" ? "bg-[#ff8516] text-white" : "hover:bg-[#1a1d29]"}`}
            >
              1d
            </button>
          </div>
        ...
        <div className="flex-1 flex flex-col">
              ...
              <div className="flex items-center gap-4">
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 hover:bg-[#1a1d29] px-2 py-1 rounded transition-colors"
                  >
                    <span className="text-2xl">{selectedAsset.flag}</span>
                    <div className="text-left">
                      <span className="font-bold text-lg block">{selectedAsset.name}</span>
                  ...
                  {selectedAsset.isMock && (
                    <span className="ml-4 px-2 py-0.5 bg-yellow-500/20 text-yellow-500 text-[10px] font-bold uppercase tracking-wider rounded border border-yellow-500/30">
                      Simulated Data
                    </span>
                  )}
                  ...
                  {dropdownOpen && (
                    <>
                      ...
                      />
                      <div className="absolute top-full left-0 mt-2 bg-[#1a1d29] border border-[#2a2d3a] rounded-lg shadow-xl z-50 min-w-[280px] max-h-[400px] overflow-y-auto">
                        {currentAssets.map((asset, idx) => (
                          <div key={idx} className="flex items-center border-b border-[#2a2d3a] last:border-b-0">
                            <button
                              onClick={() => {
                                setSelectedAsset(asset);
                                setDropdownOpen(false);
                              }}
                              className={`flex-1 px-4 py-3 text-left text-sm hover:bg-[#2a2d3a] transition-colors flex items-center gap-3 ${selectedAsset?.name === asset.name ? 'bg-[#ff8516]/20 text-[#ff8516]' : 'text-white'
                                }`}
                            >
                              <span className="text-xl">{asset.flag}</span>
                              <span className="font-semibold">{asset.name}</span>
                              <span className="text-xs text-gray-400 ml-auto">
                                ${asset.buy > 0 ? asset.buy.toFixed(2) : '--'}
                              </span>
                            </button>
                            <button
                              onClick={() => {
                                if (!openTabs.find(t => t.name === asset.name)) {
                                  setOpenTabs([...openTabs, asset]);
                                }
                                setDropdownOpen(false);
                              }}
                              className="px-3 py-3 hover:bg-[#2a2d3a] text-gray-400 hover:text-orange-500 transition-colors"
            ...
            {selectedAsset ? (
              ...
              <div className="flex items-center justify-center h-full text-gray-400">
                <div className="text-center">
                  ...
                  <div className="text-lg">Select an asset to view chart</div>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-[#0d0f15] border-t border-[#2a2d3a] h-[140px] flex flex-col">
            <div className="flex items-center gap-4 px-4 border-b border-[#2a2d3a] shrink-0">
              <button
                onClick={() => setBottomTab("positions")}
                className={`py-2 text-xs font-semibold border-b-2 transition-colors ${bottomTab === "positions"
              ...
              <button
                ...
                className={`py-2 text-xs font-semibold border-b-2 transition-colors ${bottomTab === "history"
                  ? "text-[#ff8516] border-[#ff8516]"
                  : "text-gray-400 border-transparent hover:text-white"
                  }`}
              >
              ...
              {bottomTab === "positions" && (
                <div className="p-2">
                  {positions.length === 0 ? (
                    ...
                    <div className="space-y-1" ref={positionsListRef}>
                      {positions.map((position) => (
                        <div
                          key={position.id}
                          className="flex items-center justify-between bg-[#1a1d29] rounded px-3 py-2 text-xs"
                        >
                          <div className="flex items-center gap-3">
                            <div
                              className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${position.direction === "buy"
                            ...
                            <span className="text-gray-400">Current: {position.currentPrice.toFixed(2)}</span>
                            <span
                              className={`font-bold ${position.pnl >= 0 ? "text-[#00c853]" : "text-[#ff3b30]"
                                }`}
                            >
                          ...
                          <button
                            ...
                            className="px-3 py-1 bg-[#ff8516] hover:bg-orange-hover rounded text-[10px] font-semibold"
                          >
                            Close
                          </button>
                        </div>
                    ...
                    <div className="space-y-1">
                      {history.map((trade) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div
                              className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${trade.direction === "buy"
                                ? "bg-[#00c853]/20 text-[#00c853]"
                                : "bg-[#ff3b30]/20 text-[#ff3b30]"
                                }`}
                            >
                            ...
                            <span className="text-gray-400">Entry: {trade.entryPrice.toFixed(2)}</span>
                            <span className="text-gray-400">Exit: {trade.exitPrice.toFixed(2)}</span>
                            <span className="text-gray-400 text-[10px]">{trade.closedAt}</span>
                          </div>
                          <span
                            className={`font-bold ${trade.pnl >= 0 ? "text-[#00c853]" : "text-[#ff3b30]"
                              }`}
                          >
        ...
        {/* Right Sidebar - Assets List + TRADING CONTROLS */}
        <div className="w-[420px] bg-[#0d0f15] border-l border-[#2a2d3a] flex flex-col">
          ...
          {/* Trading Controls - YAHAN SHIFT KIYA HAI */}
          {selectedAsset && (
            <div className="border-b border-[#2a2d3a] p-4 bg-[#1a1d29]">
              <h3 className="text-sm font-semibold mb-3 text-gray-300">Trade Controls</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span>Available: ${currentBalance.toFixed(2)}</span>
                  <span>
                    {tradeAmount > 0
                      ? `Remaining after trade: $${Math.max(0, (currentBalance - tradeAmount)).toFixed(2)}`
                      : ""}
                  </span>
                </div>
                  ...
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setTradeAmount(Math.max(0, value));
                      }}
                      className="w-full px-3 py-2 bg-[#0d0f15] border border-[#2a2d3a] rounded text-white focus:outline-none focus:border-[#ff8516] text-sm"
                    />
                  </div>
                  <div className="w-24">
                    ...
                    <select
                      ...
                      className="w-full px-2 py-2 bg-[#0d0f15] border border-[#2a2d3a] rounded text-white focus:outline-none focus:border-[#ff8516] text-sm"
                    >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    disabled={
                      selectedAsset.sell === 0 ||
                      tradeAmount <= 0 ||
                      tradeSubmitting === "sell"
                    }
                  ...
                  >
                    <div className="text-xs text-white/70 mb-1">
                      {tradeSubmitting === "sell" ? "Selling..." : "SELL"}
                    </div>
                    <div className="text-sm flex items-center justify-center gap-2">
                      {tradeSubmitting === "sell" && (
                        <span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" />
                      )}
                      {selectedAsset.sell > 0 ? selectedAsset.sell.toFixed(selectedAsset.sell > 100 ? 2 : 5) : '...'}
                    </div>
                  </button>
                  <button
                  ...
                  >
                    <div className="text-xs text-white/70 mb-1">
                      {tradeSubmitting === "buy" ? "Buying..." : "BUY"}
                    </div>
                    <div className="text-sm flex items-center justify-center gap-2">
                      {tradeSubmitting === "buy" && (
                        <span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" />
                      )}
                      {selectedAsset.buy > 0 ? selectedAsset.buy.toFixed(selectedAsset.buy > 100 ? 2 : 5) : '...'}
                    </div>
                  </button>
                </div>
          ...
          {/* Categories */}
          <div className="shrink-0 border-b border-[#2a2d3a]">
            ...
            <div className="sticky top-0 bg-[#0d0f15] border-b border-[#2a2d3a] z-10">
              {categories.map((category) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-3 hover:bg-[#1a1d29] transition-colors ${selectedCategory === category.id ? "bg-[#1a1d29]" : ""
                    }`}
                >
                  <div className="flex items-center gap-3">
              ...
              {marketLoading && filteredAssets.length === 0 ? (
                ...
                filteredAssets.map((asset, index) => (
                  <button
                    ...
                    className={`w-full px-4 py-3 flex items-center justify-between hover:bg-[#1a1d29] transition-colors border-b border-[#2a2d3a] ${selectedAsset?.name === asset.name ? "bg-[#1a1d29]" : ""
                      }`}
                  >
                    ...
                    <div className="flex items-center gap-6 text-sm">
                      ...
                      <span className={`w-16 text-right font-semibold ${asset.positive ? "text-[#00c853]" : "text-[#ff3b30]"
                        }`}>
          ...
          <div className="border-t border-[#2a2d3a] bg-[#0d0f15]">
            <div className="flex border-b border-[#2a2d3a]">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm ${rightTab === "information" ? "text-[#ff8516] border-b-2 border-[#ff8516]" : "text-gray-400"
                  }`}
              >
                <Info size={16} />
                Information
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm ${rightTab === "news" ? "text-[#ff8516] border-b-2 border-[#ff8516]" : "text-gray-400"
                  }`}
              >
                <Newspaper size={16} />
                News
        ...
        <DialogContent className="bg-[#1a1d29] border-[#2a2d3a] text-white">
          <DialogHeader>
            <DialogTitle className="text-xl font-semibold">
              {tradeConfirmation?.direction === "buy" ? "Buy" : "Sell"} order placed
            </DialogTitle>
            <DialogDescription className="text-gray-400">
              Your {tradeConfirmation?.account} account position has been opened successfully.
            </DialogDescription>
          </DialogHeader>
          ...
          {tradeConfirmation && (
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-400">Asset</span>
                <span className="font-semibold">{tradeConfirmation.asset}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Direction</span>
                <span className="font-semibold uppercase">
                  {tradeConfirmation.direction}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Invested</span>
                <span className="font-semibold">
                  ${tradeConfirmation.amount.toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Entry price</span>
                <span className="font-semibold">
                  {tradeConfirmation.entryPrice.toFixed(
                    tradeConfirmation.entryPrice > 100 ? 2 : 5
                  )}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {tradeConfirmation.quantity.toFixed(4)}
                </span>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row gap-2 pt-6">
            <UiButton
              variant="outline"
              className="w-full sm:flex-1 border-[#2a2d3a] bg-transparent text-white hover:bg-[#2a2d3a] whitespace-nowrap justify-center"
              onClick={() => {
                setBottomTab("positions");
                setTradeConfirmation(null);
                positionsListRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
            ...
            <UiButton
              className="w-full sm:flex-1 bg-[#00c853] hover:bg-[#00d65f] whitespace-nowrap justify-center"
